...
> **重要提示**：根据 `xpai.yaml` 内的 SSH 配置，确保集群内所有主机的 SSH 端口、root 密码保持一致。

#### 1.4 GPU 驱动准备

> **说明**：本工具默认不安装显卡驱动。由于离线环境显卡驱动和版本要求难以统一控制，安装过程不包含驱动安装。请在目标主机上自行安装符合要求的 GPU 驱动。
...
> **说明**：nerdctl 是 Docker 的替代工具，适用于 containerd 容器运行时环境。
...
| 命令 | 说明 |
|------|------|
| `make package` | 构建离线镜像包 |
| `make nerdctl` | 安装 nerdctl 工具 |
| `./xpaictl.sh --config xpai.yaml --masters <IP>` | 单节点部署 |
| `./xpaictl.sh --config xpai.yaml --masters <IP> --nodes <IPs> --offline` | 离线多节点部署 |
| `sealos reset` | 卸载平台 |
